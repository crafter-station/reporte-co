...
import { type Category, departamentoCentroid, isCategory } from "./taxonomy";
 ...
 * transforms: scrub PII from the text, coarsen (or departamento-centroid) the
...
async function autoPublish(id: string, input: IngestInput): Promise<void> {
  ...
  // departamento centroid, else leave the pin off (no departamento → no public location).
  const coords =
    input.lat != null && input.lng != null
      ? coarsenCoords(input.lat, input.lng)
      : ((input.departamento
          ? departamentoCentroid(input.departamento)
          : null) ?? {
          lat: null,
          lng: null,
        });
